...
/*
NAME: Ananth Adhikarla
SID: 103462848

NAME: Matt McCarthy
SID: 104201581

There are a lot of comments here. Aside from explaining everything I did, I also
read through all pre-written code and tried to explain everything I wasn't immediately
sure of in order to get a better understanding of such material. I also left all of my
debugging code intact and commented it all out. I figured it would be useful for you to
see my thought processes in order to more accurately grade me on this assignment in case 
there are errors.
*/
...
// dllist_node<T>
//
// Having nodes in a list that don't store any data wouldn't be of much
...
public:
  ...
  explicit dllist_node(T const& datum):
    dllist_node::dllist_node_ptr_only{nullptr, nullptr},
    datum_{datum}
  {}
  ...
  dllist_node(T const& datum, dllist_node* prev, dllist_node* next):
    dllist_node::dllist_node_ptr_only{prev, next},
    datum_{datum}
  {}
  ...
  dllist_node(T const& datum, xorptr_type const& xp):
    dllist_node::dllist_node_ptr_only{xp},
    datum_{datum}
  {}
  ...
  dllist_node(T&& datum, xorptr_type&& xp):
    dllist_node::dllist_node_ptr_only{std::move(xp)},
    datum_{std::move(datum)}
  {}
  ...
  dllist_node(dllist_node&& n):
    dllist_node::dllist_node_ptr_only{std::move(n)},
